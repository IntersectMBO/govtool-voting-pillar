...
const hasSupportedSpecVersion = (metadata: unknown): boolean => {
...
export const parseSurveyLink = (metadata: unknown): SurveyLink | null => {
  if (!hasSupportedSpecVersion(metadata)) return null;
  return parseCip179Link(metadata).surveyRef;
};
...
export const enrichDefinition = async (
  ...
  if (!definition.contentAnchor) return definition;
