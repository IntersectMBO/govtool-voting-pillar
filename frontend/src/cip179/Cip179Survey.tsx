...
export type CustomQuestionRenderers = Readonly<
  Record<string, CustomQuestionRenderer>
>;
...
export const customMethodKey = (
  ...
  `${question.methodSchema.uri}#${bytesToHex(question.methodSchema.hash)}`;
...
const ratingValues = (question: Extract<Question, { type: 'rating' }>) => {
  ...
  const { min, max, step = 1n } = question.scale.constraints;
  const values: { label: string; value: bigint }[] = [];
  for (let value = min; value <= max && values.length < 100; value += step) {
    values.push({ label: value.toString(), value });
  }
  return values;
};
  ...
  switch (question.type) {
    ...
    case 'pointsAllocation': {
      ...
      return {
        ...
        questionIndex,
        allocations: points.flatMap((amount, optionIndex) =>
          amount > 0 ? [{ optionIndex, points: amount }] : []
        ),
      };
    }
    case 'rating': {
...
export const Cip179Survey = ({
  ...
  useEffect(() => {
    const nextLink = parseSurveyLink(proposal.metadataJson);
    setLink(nextLink);
    setDefinition(null);
    setError(null);
    setWarning(null);
    ...
    if (!nextLink) return;
    if (!codec) {
      setError('The host application has not configured a metadatum codec');
      return;
    }
    let active = true;
    axios
      .get<SurveyDefinitionEnvelope>(
        `${apiUrl}/survey/definition/${nextLink.txId}/${nextLink.index}`
      )
      .then(async ({ data }) => {
      ...
      .catch((reason: unknown) => {
        if (active) {
          setError(
            reason instanceof Error ? reason.message : 'Survey unavailable'
          );
        }
      });
    return () => {
  ...
  }, [apiUrl, codec, proposal.expiryEpochNo, proposal.metadataJson]);
  ...
  const responseState = useMemo<Cip179Participation>(() => {
    ...
    const response: SurveyResponse = {
      ...
      surveyRef: {
        txId: Uint8Array.from({ length: 32 }, (_, index) =>
          parseInt(link.txId.slice(index * 2, index * 2 + 2), 16)
        ),
        index: link.index,
      },
      role: Role.DRep,
      credential: {
        type: 'key',
        keyHash: Uint8Array.from({ length: 28 }, (_, index) =>
          parseInt(dRepId.slice(index * 2, index * 2 + 2), 16)
        ),
      },
  ...
  return (
    <Box
      sx={{ borderTop: 1, borderColor: 'divider', mt: 4, pt: 3, width: '100%' }}
    >
      <Typography variant="h6">
        {definition.title || 'Linked survey'}
      </Typography>
      {definition.description && (
        <Typography sx={{ mt: 1 }}>{definition.description}</Typography>
      )}
      {warning && (
        <Alert severity="warning" sx={{ mt: 2 }}>
          External presentation unavailable: {warning}. Index-based labels are
          shown.
        </Alert>
      )}
      {!eligible ? (
      ...
      {eligible && participating && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 2 }}>
          {definition.questions.map((question, index) => {
            const labels =
              'options' in question ? optionLabels(question.options) : [];
              ...
              setTouched((current) => {
                ...
                const empty =
                  ...
                  (question.type === 'rating' &&
                    Array.isArray(value) &&
                    value.every((rating) => rating === null));
              ...
              >
                ...
                {question.type === 'singleChoice' && (
                  <RadioGroup
                    value={values[index] ?? ''}
                    onChange={(event) =>
                      markTouched(Number(event.target.value))
                    }
                  >
                ...
                {(question.type === 'multiSelect' ||
                  ...
                  labels.map((label, optionIndex) => {
                    const selected =
                      (values[index] as number[] | undefined) ?? [];
                ...
                {question.type === 'numericRange' && (
                  <TextField
                    ...
                    helperText={`${question.constraints.min.toString()} to ${question.constraints.max.toString()}${
                      question.constraints.step
                        ? `, step ${question.constraints.step.toString()}`
                        : ''
                    }`}
                  />
                )}
